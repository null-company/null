...
ClientNetworkManagerTest::ClientNetworkManagerTest(sf::IpAddress address, uint16_t port)
        : client(address, port) {
}

std::queue<net::GameMessage> &ClientNetworkManagerTest::subscribe(int playerId, int messageId) {
...
net::GameMessage ClientNetworkManagerTest::receiveMessage() {
    return client.receiveGameMessage();
}


void ClientNetworkManagerTest::multiplexMessage(net::GameMessage &gameMessage) {
...
std::queue<net::GameMessage> &ClientNetworkManagerTest::getQueue(int playerId) {
    ensurePlayerChannel(playerId);
    return channels.at(playerId);
}

std::set<int> &ClientNetworkManagerTest::getSubscribedSet(int messageId) {
    ensureMessageIdSet(messageId);
    return messageIdToSubscribedPlayersIds.at(messageId);
}

void ClientNetworkManagerTest::ensureMessageIdSet(int messageId) {
...
void ClientNetworkManagerTest::ensurePlayerChannel(int playerId) {
...
void ClientNetworkManagerTest::unsubscribe(int playerId, int messageId) {
    ensurePlayerChannel(playerId);
    ensureMessageIdSet(messageId);
...
Client &ClientNetworkManagerTest::getClient() {
    return client;
}
