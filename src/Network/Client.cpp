...
net::NetMessage Client::receiveNetMessage(sf::TcpSocket &socket) {
...
void Client::sendNetMessage(sf::Socket &socket, const net::NetMessage &message) {
    LOGD << "Sending Some Message";
    sf::Packet packet;
    packet << message.SerializeAsString();
    if (clientCommandSocket.send(packet) != sf::Socket::Done) {
