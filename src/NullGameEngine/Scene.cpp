...
namespace null {
    ...
    serial::Scene Scene::prefabSerialize() {
        serial::Scene serializeScene;
        serial::BasicGameObject &gameObject = *(serializeScene.mutable_game_object()->mutable_basic_game_object());
        for (auto &childObject: this->gameObjects) {
            *gameObject.add_children_objects() = childObject->prefabSerialize();
        }
